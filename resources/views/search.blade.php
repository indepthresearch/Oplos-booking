...
{{-- Search Panel --}}
   ...
   <p>{{$listings->total()}} Result(s) for '{{request()->input('query')}}'</p>
   ...
   @if ($listings->total() > 0)
    <div class="living_middle">
	   ...
	   @foreach($listings->slice(0, 6) as $listing)
   ...
   <!-- <table class="table table-bordered table-striped">
            ...
            <tbody>
                 @foreach($listings as $listing)
                    <tr>
                        <th><a href="{{route('product.show',$listing->property_name)}}">{{ $listing->property_name }}</a></th>
                        <td>{{ $listing->location }}</td>
                        <td>{{ str_limit($listing->Description, 80) }}</td>
                        <td>{{ $listing->Capacity }}</td>
                        <td>{{ $listing->price_per_hall }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table> -->
